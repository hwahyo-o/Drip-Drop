...
- 현재 위치 좌표: 소수점 뒤 20자리 문자열을 `latText`, `lngText`로 보존
...
현재 위치는 브라우저 Geolocation API에서 가져옵니다. 앱은 짧은 시간 동안 여러 측정값을 받아 가장 나은 브라우저 좌표를 선택하고, 좌표 문자열을 소수점 뒤 20자리까지 `latText`, `lngText`에 보존합니다.

지도 표시는 Leaflet/Naver SDK가 요구하는 숫자 좌표를 사용합니다. JavaScript 숫자 타입과 브라우저 위치 센서의 실제 정밀도에는 한계가 있으므로, 소수점 20자리 문자열 보존이 실제 센서 정확도 20자리를 보장하지는 않습니다.

요청에 따라 현재 위치 마커 주변의 정확도 반경 원과 `정확도 약 Xm` 팝업 문구는 표시하지 않습니다.
