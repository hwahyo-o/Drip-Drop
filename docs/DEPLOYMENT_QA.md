...
- `apiKey`가 비어 있으면 GitHub Repository secrets에 `VITE_FIREBASE_*` 값이 등록되지 않았거나, 등록 후 Pages 재배포가 아직 완료되지 않은 상태입니다.
...
- 현재 위치 주변에 정확도 반경 원이 표시되지 않는지 확인합니다.
- 현재 위치 팝업에 `정확도 약 Xm` 문구가 표시되지 않는지 확인합니다.
- 개발자 도구에서 위치 객체를 확인할 경우 `latText`, `lngText`가 소수점 뒤 20자리 문자열로 보존되는지 확인합니다.
